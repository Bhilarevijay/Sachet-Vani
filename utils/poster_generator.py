...
def generate_missing_poster(missing_child, base_url="https://sachet.onrender.com"):
    ...
    try:
        ...
        if font_path:
            ...
            detail_font = ImageFont.truetype(font_path, 60)
        else:
    ...
    if missing_child.photo_filename:
        try:
            ...
            if missing_child.photo_filename.startswith('http'):
                ...
                photo_path = os.path.join('static', 'uploads', 'photos', missing_child.photo_filename)
    ...
    if hasattr(missing_child, 'location_subcategory') and missing_child.location_subcategory:
    ...
    for label, value in details:
        ...
        max_width = width - 400
        wrapped_text = wrap_text(str(value), detail_font, max_width, draw)
    ...
    contact_text = missing_child.emergency_contact or "Contact authorities"
...
def poster_to_bytes(poster, format='PNG'):
    """Convert PIL Image to bytes for HTTP response"""
    buffer = BytesIO()
    poster.save(buffer, format=format)
    buffer.seek(0)
    return buffer
