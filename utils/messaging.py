...
def send_discord_alert(message, photo_url=None, webhook_url=None):
    ...
    try:
        ...
        if not url:
            print("⚠️ Discord not configured (missing DISCORD_WEBHOOK_URL)")
            return False
...
def broadcast_alert(message, photo_url=None, sms_func=None):
    ...
    Args:
        message: Text message to send
        photo_url: Optional photo URL to attach
        sms_func: Optional SMS sending function (e.g., send_sms_alert)
    ...
    if sms_func:
        try:
            ...
            results['sms'] = sms_count > 0 if isinstance(sms_count, int) else bool(sms_count)
        except Exception as e:
